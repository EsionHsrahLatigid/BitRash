...
The audio callback owns no file, network, logging, lock, or heap allocation work in steady state. `BitRashDSP` sanitizes non-finite input, smooths continuous targets, applies discrete bit/deci clock choices deterministically, caps error-feedback/noise-shaping paths, and clamps output finite.
